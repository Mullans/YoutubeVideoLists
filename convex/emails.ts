...
export const sendInvitationEmail = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      console.log("📨 Attempting to send email via direct HTTP request...");
      ...
      if (!response.ok) {
        console.error("❌ Failed to send invitation email:", responseText);
        throw new Error(`Failed to send invitation email: ${response.status} ${responseText}`);
      }
